...
function DoctorStatusBadge({ availability }) {
  return <span className={`status-pill ${availability.tone}`}>{availability.label}</span>;
}
...
function BookButton({ availability, onClick }) {
  ...
  return <button className="book-btn" onClick={onClick}>Book Token</button>;
}
...
export default function HospitalFlow({ user, isGuest, onLogout, displayName, initialCity, lang, t, familyMembers }) {
  ...
  const [selectedFamilyMember, setSelectedFamilyMember] = useState(displayName || 'Self (Primary)');
  ...
  const startVoiceSearch = () => {
    ...
    recognition.onresult = (event) => {
      ...
      if (spokenText.includes('kan') || spokenText.includes('ear') || spokenText.includes('throat') || spokenText.includes('nose') || spokenText.includes('cough')) {
        setActiveSpecialty('ENT Specialist');
        setSearchTerm('');
      } else if (spokenText.includes('tooth') || spokenText.includes('teeth') || spokenText.includes('dental')) {
        setActiveSpecialty('Dentist');
        setSearchTerm('');
      } else if (spokenText.includes('skin') || spokenText.includes('rash') || spokenText.includes('allergy')) {
        setActiveSpecialty('Dermatologist');
        setSearchTerm('');
      } else if (spokenText.includes('child') || spokenText.includes('baby')) {
        setActiveSpecialty('Pediatrician');
        setSearchTerm('');
      } else if (spokenText.includes('bone') || spokenText.includes('joint') || spokenText.includes('knee')) {
        setActiveSpecialty('Orthopedic');
        setSearchTerm('');
      } else if (spokenText.includes('pregnancy') || spokenText.includes('periods') || spokenText.includes('women')) {
        setActiveSpecialty('Gynecologist');
        setSearchTerm('');
      } else if (spokenText.includes('fever') || spokenText.includes('headache') || spokenText.includes('weakness')) {
  ...
  return (
    <div className="flow-page" style={{ width: '100%', maxWidth: '100vw', boxSizing: 'border-box', overflowX: 'hidden' }}>
      <div className="flow-content" style={{ paddingBottom: '90px', width: '100%', maxWidth: '650px', margin: '0 auto', boxSizing: 'border-box' }}>
        
        {/* HERO BANNER */}
        <div style={{ background: 'linear-gradient(135deg, #e6f4ea 0%, #daf2e1 100%)', border: '1px solid rgba(16, 185, 129, 0.2)', borderRadius: '20px', padding: '20px 22px', marginBottom: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: '0 4px 15px rgba(16, 185, 129, 0.06)' }}>
          <div style={{ flex: 1, paddingRight: '12px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
              <span style={{ fontSize: '18px' }}>☀️</span>
              <h3 style={{ fontFamily: 'Fraunces, serif', fontSize: '16.5px', color: '#0b332c', margin: 0 }}>{getDynamicGreeting()}</h3>
            </div>
            <p style={{ fontSize: '12px', fontWeight: '600', color: '#134e44', margin: '0 0 4px' }}>Your health, our priority</p>
            <p style={{ fontSize: '11px', color: '#475569', margin: 0, lineHeight: '1.4' }}>Find trusted doctors & quality care near you</p>
          </div>
          <div style={{ width: '95px', height: '65px', background: 'rgba(255,255,255,0.7)', borderRadius: '14px', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 2px 8px rgba(0,0,0,0.04)', flexShrink: 0, border: '1px solid rgba(16,185,129,0.15)' }}>
        ...
        <div style={{ marginBottom: '16px', display: 'flex', width: '100%', boxSizing: 'border-box' }}>
          <div style={{ position: 'relative', width: '100%' }}>
            <button
              ...
              style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#fff', border: '1px solid #e2e8f0', padding: '12px 16px', borderRadius: '14px', fontSize: '13.5px', fontWeight: '600', color: '#0b332c', cursor: 'pointer', boxSizing: 'border-box', boxShadow: '0 2px 6px rgba(0,0,0,0.02)' }}
            >
            ...
            {showCityPicker && (
              <div style={{ position: 'absolute', left: 0, right: 0, top: '52px', background: '#fff', border: '1px solid #e2e8f0', borderRadius: '14px', boxShadow: '0 10px 25px rgba(0,0,0,0.1)', zIndex: 100, overflow: 'hidden' }}>
                <button
                  style={{ width: '100%', textAlign: 'left', padding: '12px 16px', background: !currentCity ? '#f0fdf4' : 'transparent', border: 'none', borderBottom: '1px solid #f1f5f9', fontSize: '13px', fontWeight: '600', cursor: 'pointer', color: '#0b332c' }}
                ...
                {allCities.map((c) => (
                  <button
                    key={c}
                    style={{ width: '100%', textAlign: 'left', padding: '12px 16px', background: currentCity === c ? '#f0fdf4' : 'transparent', border: 'none', borderBottom: '1px solid #f1f5f9', fontSize: '13px', fontWeight: '600', cursor: 'pointer', color: '#0b332c' }}
        ...
        {!selectedHospital && (
          <div>
            <div className="search-bar-wrap" style={{ display: 'flex', gap: '8px', alignItems: 'center', marginBottom: '16px', width: '100%', boxSizing: 'border-box' }}>
              <div style={{ position: 'relative', flex: 1, minWidth: 0 }}>
                <input
                  ...
                  style={{ width: '100%', paddingRight: isSearchActive ? '50px' : '16px', boxSizing: 'border-box', background: '#fff', borderRadius: '14px', border: '1px solid #e2e8f0', padding: '12px 16px', fontSize: '14px' }}
                />
                {isSearchActive && <button className="search-clear-btn" onClick={clearSearch}>Clear</button>}
              </div>
              ...
              <button
                ...
                style={{ background: isListening ? '#ef4444' : '#0b332c', color: '#fff', border: 'none', cursor: 'pointer', padding: '12px 16px', borderRadius: '14px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, boxShadow: '0 4px 12px rgba(11, 51, 44, 0.15)', transition: 'background 0.2s' }}
              >
            ...
            {/* SPECIALTY CHIPS */}
            <div className="specialty-chips" style={{ width: '100%', boxSizing: 'border-box', marginBottom: '22px' }}>
              <button
            ...
            {isSearchActive ? (
              <>
                ...
                {searching ? (
                  ...
                  <div className="doctor-list">
                    {searchResults.sort((a, b) => a.liveQueue - b.liveQueue).map((doc) => {
                            ...
                            <div className="doctor-details">
                              <div className="doctor-details-top">
                                <div>
                                  ...
                                  <p style={{ margin: '0 0 2px', fontSize: '11px', color: '#10b981', fontWeight: 700 }}>
                                    {doc.degrees || 'MBBS, General Practitioner'}
                                  </p>
                                </div>
              ...
              <>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                  <h3 className="flow-section-title" style={{ margin: 0 }}>Available Hospitals</h3>
                  <span style={{ fontSize: '12px', fontWeight: '700', color: '#10b981', cursor: 'pointer' }}>View all →</span>
                </div>
                ...
                {loadingHospitals ? (
                  ...
                  <div className="hospital-list">
                    {hospitals.map((hosp) => {
                      const isMapLink = hosp.location && (hosp.location.startsWith('http://') || hosp.location.startsWith('https://'));
                      const directionsUrl = isMapLink ? hosp.location : `https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(`${hosp.name}, ${hosp.location || ''}, ${hosp.city || ''}`)}`;
                              ...
                              <div className="hospital-meta-row">
                                {availableCounts[hosp.id] > 0 ? (
                                  <span className="doctor-available-badge">
                                    ...
                                    {availableCounts[hosp.id]} Doctor{availableCounts[hosp.id] > 1 ? 's' : ''} Available
                                  </span>
                                ) : (
                                  <span className="no-doctor-badge">Check schedule</span>
                                )}
                              </div>
            ...
            {/* TRUST BANNER */}
            <div style={{ background: '#e6f4ea', border: '1px solid rgba(16, 185, 129, 0.25)', borderRadius: '16px', padding: '16px 20px', marginTop: '24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <span style={{ fontSize: '24px' }}>📋</span>
                <div>
                  <h4 style={{ fontFamily: 'Fraunces, serif', fontSize: '14.5px', color: '#0b332c', margin: '0 0 2px' }}>Book with confidence</h4>
                  <p style={{ fontSize: '11.5px', color: '#475569', margin: 0 }}>Verified doctors, Real-time queue info, Secure appointments</p>
                </div>
              </div>
      ...
      {/* CONFIRMATION & CARE CIRCLE MODAL */}
      {pendingBooking && (
        <div className="ticket-overlay" style={{ position: 'fixed', inset: 0, zIndex: 2000, background: 'rgba(6, 43, 37, 0.65)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
          <div className="ticket-card" style={{ background: '#fff', width: '100%', maxWidth: '420px', borderRadius: '24px', padding: '24px', boxShadow: '0 25px 50px rgba(0,0,0,0.25)', boxSizing: 'border-box', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
              <div>
                <h2 style={{ fontFamily: 'Fraunces, serif', fontSize: '20px', color: '#0b332c', margin: '0 0 4px' }}>Confirm Appointment</h2>
                <p style={{ fontSize: '13px', color: '#64748b', margin: 0 }}>Dr. {pendingBooking.doc.name} • <span style={{ color: '#10b981', fontWeight: '600' }}>{pendingBooking.doc.specialty}</span></p>
              </div>
            ...
            {/* CARE CIRCLE SELECTOR */}
            <div style={{ marginBottom: '16px' }}>
              <label style={{ fontSize: '12px', fontWeight: '700', color: '#0b332c', display: 'block', marginBottom: '6px' }}>Booking For (Care Circle):</label>
              <select
              ...
              >
                ...
                {familyMembers && familyMembers.map((member, index) => (
                  <option key={index} value={member.name}>{member.name} ({member.relation})</option>
                ))}
              </select>
            </div>

            <div style={{ marginBottom: '16px' }}>
              <label style={{ fontSize: '12px', fontWeight: '700', color: '#0b332c', display: 'block', marginBottom: '6px' }}>Contact Phone Number:</label>
              <input
            ...
            {selectedPayment === 'upi' && hospitalUpi && (
              <div style={{ background: '#f8f6f0', padding: '14px', borderRadius: '14px', marginBottom: '16px', border: '1px solid #e2e8f0' }}>
                <p style={{ fontSize: '12.5px', margin: '0 0 8px', color: '#0b332c' }}>Pay via UPI to: <strong>{hospitalUpi}</strong></p>
                <input
            ...
            {bookingError && <p style={{ color: '#ef4444', fontSize: '12px', margin: '0 0 12px', fontWeight: '600', background: '#fef2f2', padding: '8px', borderRadius: '8px' }}>{bookingError}</p>}
            ...
            <button onClick={() => setPendingBooking(null)} style={{ width: '100%', background: '#fff', color: '#64748b', border: '1px solid #e2e8f0', padding: '11px', borderRadius: '14px', fontSize: '13px', fontWeight: '700', cursor: 'pointer' }}>Cancel</button>
          </div>
        </div>
      )}

      {showCelebration && (
        <div className="celebration-overlay" style={{ position: 'fixed', inset: 0, zIndex: 3000, background: 'rgba(6, 43, 37, 0.65)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
          <div style={{ background: '#fff', width: '100%', maxWidth: '360px', borderRadius: '24px', padding: '32px 24px', boxShadow: '0 20px 40px rgba(0,0,0,0.25)', boxSizing: 'border-box', textAlign: 'center', position: 'relative', overflow: 'hidden', animation: 'mediq-pop-in 0.35s ease' }}>
            <h2 style={{ fontFamily: 'Fraunces, serif', fontSize: '20px', color: '#0b332c', margin: '0 0 8px' }}>Booking Confirmed!</h2>
            <p style={{ fontSize: '13.5px', color: '#475569', margin: '0 0 24px', lineHeight: 1.5 }}>Your queue token has been booked successfully for <strong>{selectedFamilyMember}</strong>.</p>
            <button onClick={() => setShowCelebration(false)} style={{ width: '100%', background: '#10b981', color: '#fff', border: 'none', padding: '13px', borderRadius: '12px', fontSize: '14px', fontWeight: '700', cursor: 'pointer' }}>Great, Thanks!</button>
          </div>
        </div>
      )}
    </div>
